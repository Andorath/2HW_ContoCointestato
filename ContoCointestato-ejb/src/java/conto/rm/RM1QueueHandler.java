...
public class RM1QueueHandler implements RM1QueueHandlerLocal
{
    ...
    private void initialize()
    {
        ...
        ts.createIntervalTimer(10, 1000, new TimerConfig());
    }
    ...
    public void commit(Commitment c)
    {
        ...
        if(pr != null)
        {
            if (pr.getTimestamp().compareTo(c.getTimestamp()) > 0)
            {
                ...
                {
                    failure("RM1");
                    throw new Exception("[RM1] ARREO Exception nel comparare " + pr.getTimestamp() + " > " + c.getTimestamp());
                } catch (Exception ex)
                {
                    Logger.getLogger(RM1QueueHandler.class.getName()).log(Level.SEVERE, null, ex);
                }
